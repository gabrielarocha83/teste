ALTER Procedure [dbo].[spCobranca_EfetivaPorCultura]
	@EmpresaId char(1),
...
BEGIN
	...
	CREATE TABLE #Titulos (ContaClienteId uniqueidentifier, DataVencimento datetime, Valor decimal(18,9), DiretoriaId varchar(10) COLLATE DATABASE_DEFAULT, Diretoria varchar(120) COLLATE DATABASE_DEFAULT, CTC varchar(10) COLLATE DATABASE_DEFAULT, StatusCobrancaID uniqueidentifier, CodigoCulturaSAP varchar(3) COLLATE DATABASE_DEFAULT, DescricaoCultura varchar(20) COLLATE DATABASE_DEFAULT)
	...
	CREATE TABLE #Resultado (Tipo char(2) COLLATE DATABASE_DEFAULT, Chave varchar(120) COLLATE DATABASE_DEFAULT, Nome varchar(120) COLLATE DATABASE_DEFAULT, Dias30 decimal(18,9), Dias60 decimal(18,9), Dias90 decimal(18,9), Dias180 decimal(18,9), DiasMais decimal(18,9), Total decimal(18,9), Percentual decimal(12,2))
	...
	Insert Into #Titulos (ContaClienteId, DataVencimento, Valor,CTC,StatusCobrancaID,CodigoCulturaSAP,DescricaoCultura)
	...
	CASE T.TipoDocumento 
		...
		ELSE OV.CodigoCtc
		END,	
	IsNull(T.StatusCobrancaID,@StatusCobrancaIDPadrao), RTrim(IsNull(IOV.CodigoCulturaSAP,'')) as CodigoCulturaSAP, RTrim(IsNull(IOV.DesricaoCultura,'')) as DesricaoCultura
	From Titulo T
	...
	Left Join ItemOrdemVenda IOV On (IOV.OrdemVendaNumero = T.OrdemVendaNumero And IOV.Item = T.OrdemVendaItem)
	Left Join OrdemVenda OV On (OV.Numero=IOV.OrdemVendaNumero)
	...
	Update T Set T.DiretoriaId = IsNull(D.DiretoriaId,'')
	From #Titulos T
	...
	Select Distinct T.CodigoCulturaSAP, T.DescricaoCultura
	From #Titulos T
	...
	From #Resultado R
	Inner Join (Select CodigoCulturaSAP, DescricaoCultura, IsNull(Sum(Valor),0) as Valor From #Titulos Where DateDiff(Day,DataVencimento,getdate()) <= 30 Group By CodigoCulturaSAP, DescricaoCultura) TX On (TX.CodigoCulturaSAP = R.Chave And TX.DescricaoCultura = R.Nome)

	Update R Set R.Dias60 = TX.Valor
	From #Resultado R
	Inner Join (Select CodigoCulturaSAP, DescricaoCultura, IsNull(Sum(Valor),0) as Valor From #Titulos Where DateDiff(Day,DataVencimento,getdate()) > 30 And DateDiff(Day,DataVencimento,getdate()) <= 60 Group By CodigoCulturaSAP, DescricaoCultura) TX On (TX.CodigoCulturaSAP = R.Chave And TX.DescricaoCultura = R.Nome)

	Update R Set R.Dias90 = TX.Valor
	From #Resultado R
	Inner Join (Select CodigoCulturaSAP, DescricaoCultura, IsNull(Sum(Valor),0) as Valor From #Titulos Where DateDiff(Day,DataVencimento,getdate()) > 60 And DateDiff(Day,DataVencimento,getdate()) <= 90 Group By CodigoCulturaSAP, DescricaoCultura) TX On (TX.CodigoCulturaSAP = R.Chave And TX.DescricaoCultura = R.Nome)

	Update R Set R.Dias180 = TX.Valor
	From #Resultado R
	Inner Join (Select CodigoCulturaSAP, DescricaoCultura, IsNull(Sum(Valor),0) as Valor From #Titulos Where DateDiff(Day,DataVencimento,getdate()) > 90 And DateDiff(Day,DataVencimento,getdate()) <= 180 Group By CodigoCulturaSAP, DescricaoCultura) TX On (TX.CodigoCulturaSAP = R.Chave And TX.DescricaoCultura = R.Nome)

	Update R Set R.DiasMais = TX.Valor
	From #Resultado R
	Inner Join (Select CodigoCulturaSAP, DescricaoCultura, IsNull(Sum(Valor),0) as Valor From #Titulos Where DateDiff(Day,DataVencimento,getdate()) > 180 Group By CodigoCulturaSAP, DescricaoCultura) TX On (TX.CodigoCulturaSAP = R.Chave And TX.DescricaoCultura = R.Nome)
